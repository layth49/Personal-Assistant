...
namespace Personal_Assistant.Triggers
{
    ...
    public sealed class VoiceTriggers
    {
        ...
        // Takes `speak` explicitly rather than reusing CommandContext.RunTool,
        // which is bound to speak:true for the `repeat` tool. A standing rule
        // needs both answers — see FireAsync.
        private readonly Func<string, IReadOnlyDictionary<string, string>, bool, Task<ToolResult>> runTool;
        ...
        public VoiceTriggers(
            ...
            Func<string, IReadOnlyDictionary<string, string>, bool, Task<ToolResult>> runTool,
        ...
        private async Task FireAsync(TriggerSpec spec)
        {
            ...
            // A tool that ran with nothing said is indistinguishable from one that
            // silently failed, so it voices its own result — UNLESS the rule
            // already carries a message, in which case the user has been told what
            // is happening and a second utterance is just noise.
            bool speak = string.IsNullOrWhiteSpace(spec.Message);
            ...
            await runTool(spec.RunTool, args, speak).ConfigureAwait(false);
        }
    }
}
