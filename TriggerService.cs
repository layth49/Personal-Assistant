...
namespace Personal_Assistant.Triggers
{
    ...
    // to go false again.
    //
    // Deliberately NOT built on the LLM. Gemini free tier is 5/min and 20/day on
    // this project, so anything that consults the model on a ticker kills the
    // assistant before lunch. Conditions are local predicates; the model is only
    // involved if an action chooses to involve it.
    //
