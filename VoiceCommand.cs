...
namespace Personal_Assistant.Dispatch
{
    ...
    // turn-based path was the only caller — with the Gemini Live session the
    // model speaks too, and a handler that swallows its own answer leaves the
    // model nothing to speak FROM. It doesn't fall silent; it invents. A live
    // smoke run had the handler compute 2:20 AM local while the model announced
    ...
    //            turn-based path, which is what keeps that path (and Phase 5's
    //            fallback onto it) sounding exactly as it did before.
    //   Data   — the facts. The Live session forwards these to the model, so its
    //            answer is grounded and it can still handle a follow-up like
    //            "and in Celsius?" without another tool call.
    //
    ...
    public sealed class ToolResult
    {
        ...
        // Optional SSML to synthesise INSTEAD of Speech on the turn-based path,
        // for answers whose pronunciation is the point — the prayer times carry
        // IPA phonemes because an English-trained voice has no phonemes for ع or
        // ظ and mangles the names without them. The Live path ignores this: the
        // model has its own voice and gets Speech + Data like any other result.
        ...
        /// plain-text equivalent, shown in the bubble and sent to the model.</summary>
