...
namespace Personal_Assistant.Presence
{
    ...
    public sealed class PresenceGate : IDisposable
    {
        ...
        // unprompted announcement in the middle of a conversation talks over the
        // reply the user is waiting for, and lands in the microphone that is still
        // listening for their next turn. SpeechService.Say serialises the audio
        // itself, so this is about not interrupting rather than not colliding —
        // but the result of getting it wrong is the same wasted turn.
