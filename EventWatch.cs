...
namespace Personal_Assistant.Events
{
    ...
    public sealed class EventWatch
    {
        ...
        // How many grounded lookups this watch has spent. Drives the backoff, and
        // is the number to look at when a watch has quietly eaten a day's quota.
    ...
    // How many grounded lookups a day the assistant may spend.
    //
    // Not a nicety. Verification runs on gemini-2.5-flash, which is the ONE model
    // this project is tightly rationed on — the free tier's 20-requests-a-day
    // limit lands on it and nothing else the app uses. Until now nothing touched
    // that quota, so a watch left re-checking on a tight loop would have been the
    // first thing in the app capable of exhausting a model for the whole day, and
    // it would have done it silently in the background.
    public sealed class EventWatchBudget
    {
        ...
        public int PerDay => LaithConfig.Int("EventVerifyChecksPerDay", 12, 0, 100);
    ...
    public sealed class EventWatchService
    {
            ...
            bool persist = false)
        {
            ...
            this.verify = verify ?? GeminiService.VerifyEventAsync;
