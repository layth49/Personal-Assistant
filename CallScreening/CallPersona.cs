...
namespace Personal_Assistant.CallScreening
{
    ...
    public static class CallPersona
    {
        ...
        public static string Build(string caller, bool greeted, string persona = null)
        {
            ...
            // CHECK IT WITH THE PERSON WHO SAID IT.
            //
            // The rule this replaces asked for a read-back and got one — but a
            // read-back with nothing hanging on it is decoration. The model
            // recited the message, the caller had no opening to correct it, and a
            // mis-heard name or time went into the log looking exactly as
            // confident as a right one. A narrow-band line mangles precisely the
            // things a message is FOR: numbers, times, names.
            //
            // So the read-back is now a question, and the answer can change the
            // record. Anchored on the tool's return value rather than on memory,
            // because a model reading back what it believes it wrote confirms
            // nothing — it is the notepad that has to be read aloud, not the
            // intention. take_message hands the stored text back for exactly this.
            text.Append(
                "ALWAYS CHECK THE MESSAGE BACK. Every time take_message returns, it gives you " +
                "message_as_recorded — the exact words now written down. Read THAT back to them in " +
                "one line, not what you meant to write, and ask if you have it right: \"Let me check " +
                "I've got that — <message>. Is that right?\" Say the numbers, times and names slowly " +
                "and clearly; the line is narrow and they are the parts that get mangled. ");

            text.Append(
                "If they correct ANY part of it, call take_message again with mode \"replace\" and the " +
                "whole message rewritten with their correction in it — never just the changed part, " +
                "and never with mode \"add\", which would leave the mistake sitting in the message " +
                "next to the fix. Then read the corrected version back and check again. Keep going " +
                "until they confirm it. If they only ADD something new, that is mode \"add\" as " +
                "normal. ");
            ...
            text.Append(
                "Before you call hang_up, check TWO things. Did they tell you anything at all for " +
                "Layth? If so, take_message must already have been called with it — if you cannot " +
                "point to that call, make it NOW, before hanging up. A call that ends without it is " +
                "a message he never receives. And did you read the message back and hear them agree " +
                "it was right? If not, do that now too: an unchecked message is one he may act on " +
                "wrongly, which is worse than a short call. ");
