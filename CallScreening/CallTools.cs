...
namespace Personal_Assistant.CallScreening
{
    ...
    public sealed class CallTools
    {
        ...
        private static ToolDefinition TakeMessageSchema() => ToolDefinition.Create(
            ...
            "their words rather than summarising it away. It hands back exactly what is " +
            "now written down: read that to the caller and let them correct it.",
            new ToolParameter("message", "string",
                ...
                "ready, collect before 6.\" With mode \"replace\", give the WHOLE corrected " +
                "message, not only the part that changed."),
            // Optional, and defaulting to the harmless one. See
            // CallSession.TakeMessage for why replace is the narrow path.
            new ToolParameter("mode", "string",
                "\"add\" (the default) adds this to what is already written down — use it " +
                "when the caller tells you something new. \"replace\" throws the previous " +
                "message away and keeps only this one — use it when they correct something " +
                "you read back to them, so the mistake does not travel alongside the fix.",
                Required: false,
                AllowedValues: new[] { "add", "replace" }));
