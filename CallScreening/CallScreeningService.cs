...
namespace Personal_Assistant.CallScreening
{
    ...
    //
    // The four pieces are deliberately separate — the transport knows WHEN and
    // WHICH BUTTON, the router and bridge know WHERE THE AUDIO GOES, the session
    // knows WHAT TO SAY, and this knows WHETHER and WHAT NEXT.
    //
    // What this class adds is the ORDER the pieces have to happen in, and that
    // order is almost entirely made of things that fail quietly if you get them
    // wrong:
    //
    //   1. the audio route is engaged BEFORE the call is answered, not after
    //   2. nothing is spoken until the transport says the call is really on the PC
    //   3. the machine is hushed first, because the speakers are the inbound leg
    //   4. the call is hung up BEFORE the route is put back
    //
    // WHEN IT IS ALLOWED TO ANSWER. The default follows the transport
    // (ICallTransport.AnswersOnlyMissedCalls) and "stop screening my calls" PAUSES
    // rather than disarms.
    //
    // For PHONE LINK, which answers a handset ringing in his hand, arming out loud
    // is right: an assistant that silently picks up the phone whenever the app
    // happens to be running is a much worse product.
    //
    // For GOOGLE VOICE it is precisely wrong. A call only reaches that number
    ...
    // when it mattered. A pause he forgets about would kill the feature the same
    // way, which is why it expires.
    public sealed class CallScreeningService : IDisposable
    {
        ...
        // with no desktop session must still screen calls.
        ...
        // 1 while a call is being screened. There is one call surface, so a second
        // overlapping attempt could only fight the first for it.
            ...
            Func<string, Task> announce = null)
        {
            ...
            if (LaithConfig.Bool("CallWidget", true))
            {
                ...
                widget.OnHangUp = () =>
                {
                    ...
                    Task.Run(() =>
                    {
                        ...
                        {
                            Console.WriteLine($"[call] widget hang up failed: {ex.Message}");
                        }

                        // Dismiss the card here too, rather than relying on the
                        // normal teardown. Pressing Hang up and watching the line
                        // drop while the card carries on counting is the button
                        // appearing not to work — which is precisely how it was
                        // reported. Ending twice is harmless.
            ...
            // Its own line, the way PresenceGate prints its own. The
            // missing-greeting warning is here and not only in Arm because that is
            // the difference between finding out now and finding out the moment
            ...
            Console.WriteLine(
                ...
                $"voice {CallGreeting.Voice}, over {transport.Name}, greeting {greetingPath}" +
                (File.Exists(greetingPath) ? "" : "  (MISSING — screening will refuse to arm)"));
        }

        /// <summary>
        /// Hands the call path the read-only tools it may borrow from the main
        ...
        /// <remarks>
        /// Read by PresenceGate, which must report BUSY for the duration, and by
        /// the wake-word loop in Program, which must ignore the wake word for it:
        /// nothing unprompted may be spoken while a stranger is on the line,
        /// because the inbound leg is a loopback on the speakers and the
        /// announcement would go straight down the phone — and then straight back
        /// into the recogniser as though the caller had said it.
        /// </remarks>
        ...
        {
            // The conversation is stopped first so it tears down tidily and its
            // final transcript is written, rather than being cut off by a call
            // window that vanished underneath it.
        ...
        /// <summary>Human-readable form of a location, for the tools.</summary>
        public string Describe(CallLocation location) => transport.Describe(location);
        ...
        /// </summary>
        /// <remarks>
        /// Twice upstream on 2026-08-22 a screened call hung after the conversation
        /// ended: the line stayed open, the widget counted upwards forever, and —
        /// the part that actually hurts — the capture role was left pointing at
        /// CABLE Output. That does not break the call it happened on. It breaks the
        /// NEXT one, and every dictation in between, silently.
        ///
        /// This does NOT fix a hang — it only ensures the machine is not left
        /// broken. `handling` is deliberately NOT cleared: the stuck call still
        /// owns whatever it owns, and inviting a second call into that is worse
        /// than screening nothing.
        /// </remarks>
        ...
        private async Task OnIncomingCallAsync(IncomingCall call)
        {
            ...
            {
                Console.WriteLine(
                    $"[call] {call.Caller} rang but screening is no longer armed — leaving it.");
                return;
            }
            ...
            try
            {
                ...
                //
                // An app that has already opened its audio streams is under no
                // obligation to notice that the default endpoint changed underneath
                // it, so moving the roles afterwards is a race that would be won
                // and lost silently — the call answered, the caller on a dead
                // cable, nothing in the log. Engaging first means the call finds
                // the path already in place and simply uses it.
                //
                ...
                finally
                {
                    ...
                    // engage time pointed at the AirPods, because they were
                    // connected when the call arrived — and route B had since
                    // DELETED those endpoints in order to answer. Restoring to a
                    // device that no longer exists cannot work, and the next call
                    // then refused to answer because a virtual cable had been left
                    // as the machine's default.
                    //
                    ...
                    // mid-conversation — instead of only the happy one.
            ...
            finally
            {
                ...
                // path entirely — it costs a synthesis round trip, which is time a
                // caller on the line does not have.
                ...
                // assistant refuse to screen a call that rang while it was typing —
                // trading the message it just took for the next one it would have
                // answered.
                ...
                if (toDeliver != null)
                {
                    _ = Task.Run(async () =>
                    {
                        ...
                        {
                            // Belt and braces; OnCallEndedAsync handles its own. A
                            // throw on a pool thread with nobody awaiting it takes
                            // the process down, which is not a thing a failed text
        ...
        private async Task ScreenAsync(IncomingCall call, CallAudioRoute route, Stopwatch started)
        {
            ...
            // run-up. Three extra rings is a normal experience; being answered and
            // met with nothing is not. 7.3s fits inside the ring with room to
            // spare, so the dead air moves into the ring either way.
            ...
            // THE LOCATION GATE, read again rather than inherited.
            //
            // Answer only returns OnPc having seen the page say so, so this is belt
            // and braces — but it is cheap belt and braces against the one mistake
            // that wastes a whole real-call test: a greeting spoken while the audio
            // is still on the handset is a greeting nobody hears, and it looks
            // exactly like a broken cable from the outside.
        ...
        private async Task ConverseAsync(IncomingCall call, CallAudioBridge bridge, TimeSpan budget)
        {
            ...
            // delivery, the process, or the machine does not. Its `ending` field is
            // the FIRST thing to read when a call drops — three quite different
            // causes look identical from outside, and this is what tells them
            // apart.
        ...
        /// the greeting is the thing covering the moment the line is answered, so
        /// it has to come off disk. Silent when there is nothing to do, which is
        /// the common case: a caller with no usable name never gets one.
        /// </remarks>
        private void EnsureGreetingClip(string caller)
        {
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    ...
                    await CallClipCache.TryRenderAsync(CallGreeting.Voice, line).ConfigureAwait(false);
                }
                catch (Exception ex)
                {
        ...
        // Plays the greeting DOWN THE PHONE LINE, capped.
        //
        // Not through the everyday speech path, which opens a WaveOutEvent on the
        // machine's default output — the laptop speakers — so the caller would hear
        // nothing and Layth would hear the greeting. The bridge writes to CABLE
        // Input, which VB-CABLE presents back as the call's microphone.
        private async Task PlayGreetingAsync(
            CallAudioBridge bridge, CallAudioRoute route, Stopwatch started, string caller)
        {
            // BY NAME WHEN WE HAVE ONE. Falls back silently: a caller with no name,
            // or a name nothing has been rendered for yet, gets the stock file.
            // There is no rendering on this path — the greeting is the thing
            // covering the moment the line is answered.
            ...
            using (var cap = new CancellationTokenSource(maxCall - started.Elapsed))
            {
                try
                {
                    // WAIT FOR A LISTENER, THEN A RUN-UP OF SILENCE.
                    //
                    // Measured upstream on a real call: the caller reliably heard
                    // the last two thirds of the greeting and never the start —
                    // about the first 1.8s of a 5.4s file, every time. The audio is
                    // written the instant the bridge reports itself up, but the path
                    // it is written into is not carrying yet. A fixed delay did not
                    // fix it; an ACTIVE audio session on the capture endpoint is the
                    // observable version of the same question.
        ...
        // is captured by the loopback and transcribed as though the caller had said
        // it, and the call sends the same mix to the caller. So for the length of a
        // call:
        //
        ...
        // rather than looking at the mute — and Program's wake-word loop ignores
        // the wake word outright, because a mute stops the assistant speaking
        // UNPROMPTED and a caller saying the wake word out of the speakers is a
        // prompt.
        ...
        private sealed class Hush : IDisposable
        {
            ...
            // renders the caller IS the microphone level. Measured upstream
            // 2026-08-22: the volume happened to sit at 23%, inbound arrived at
            // 0.0005 against a 0.05 target, and the bridge's 8x ceiling could not
            // close a gap that needed ~100x. The caller was audible but repeatedly
            // misheard.
            //
            ...
            public void Dispose()
            {
                if (previousVolume.HasValue)
                {
                    ...
                    {
                        Console.WriteLine(
                            $"[call] could not put the volume back to {back}%: {ex.Message}");
                    }
                }

                if (held)
                {
                    // PUT BACK WHAT WE FOUND, rather than unmuting. See
                    // PresenceGate.MuteUntil.
                ...
                _ = Task.Run(async () =>
                {
                    ...
                    {
                        Console.WriteLine($"[call] could not resume media: {ex.Message}");
                    }
                });
            }
        }
        ...
        // Next to the exe, like the call clips and keyword.table, so it survives
        ...
        public void Dispose()
        {
            ...
            // A live conversation is stopped, not abandoned: it tears down tidily
            // and the transcript is written. Hanging up is the teardown hook's job
            // (HangUpAnyCall) and putting the audio back is
            // CallAudioRouter.RestoreAll's — both registered in Program.
            Volatile.Read(ref live)?.Stop(CallEnding.Cancelled);
        }
    }

    // Where the arm state lives between runs. Same discipline as the trigger and
    // watch stores: AppData, per-write temp plus File.Replace, one save lock, and
    // every failure non-fatal — an unreadable file costs the arm, never the app.
